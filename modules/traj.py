...
def computescalarkft_parallel(f, x, G, root, outname, ncpus=10):
    if os.path.exists(root + outname + '.npy' ):
        return np.load(root + outname + '.npy')

    start = time.time()
    with Pool(ncpus) as p:
        inputs = [(f, x, G[i]) for i in range(np.shape(G)[0])]
        result = p.starmap(computescalarkft, inputs)
    print('kft of density done in ', time.time() - start)

    np.save(root + outname + '.npy', np.array(result).T)
    return np.array(result).T


def computescalarkft(f, x, G):
