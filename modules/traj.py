...
def read_dump_old(root, filename, Np, ntry):
    ...
    with open(root + filename, 'r') as f:
        ...
        for index, line in enumerate(f):
            ...
            d.append(dlist)
            # print(dlist)
            # print(index+1, Np+9, (index + 1) % (Np + 9))
            if (index + 1) % (Np + 9) == 0:
    ...
    with open(root + filename, 'r') as f:
        for indexx, lines in enumerate(f):
            ...
            if len(linesplit) > 2 and linesplit[1] == 'ATOMS':
                ...
                warnings.warn('modifica questa parte per renderla piu` generale', DeprecationWarning)
                break
...
def computekftnumba_old(root, Np, L, posox, nk, ntry, natpermol):
    start0 = time.time()
    enk = []
    dipenkx = []
    dipenky = []
...
def computekftnumba(root, Np, L, posox, nk, ntry, natpermol):
    ...
    with h5py.File(root + 'dump.h5', 'r') as dump:
        ...
        for i in range(lenght + 1, len(snap) + 1):
            ...
            warnings.warn('sto assumendo due specie solamente', DeprecationWarning)
            ...
            if int(len(chk) + 1) % int(len(snap) / 4 + 1) == 0:
                with open(root + 'output.out', 'a') as z:
                    ...
                    with open(root + 'chk.pkl', 'wb+') as g:
                        pk.dump(chk, g)
                    with open(root + 'nk1.pkl', 'wb+') as g:
                        pk.dump(n1k, g)
                    with open(root + 'nk2.pkl', 'wb+') as g:
                        pk.dump(n2k, g)
